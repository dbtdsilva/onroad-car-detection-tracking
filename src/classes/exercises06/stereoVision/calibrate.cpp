...
int FindAndDisplayChessboard(Mat image, int board_w, int board_h, vector<Point2f> *corners) {
    ...
    // Draw results
    if (true) {
        cornerSubPix(grey_image, *corners, Size(11, 11), Size(-1, -1),
...
int main(int argc, char **argv) {
    ...
    cout << "re-projection error =" << rms << std::endl;

    FileStorage fs("../CamParams.xml", FileStorage::WRITE);
    fs << "intrinsic0" << intrinsic[0] << "intrinsic1" << intrinsic[1];
    fs << "distortionCoefficients0" << distortionCoefficients[0] << "distortionCoefficients1" << distortionCoefficients[1];
    fs << "rotationMatrix" << rotationMatrix << "translationVector" << translationVector;
    fs << "essentialMatrix" << essentialMatrix << "fundamentalMatrix" << fundamentalMatrix;
    fs.release();
